...
{
    return get_proc_struct_for_pid1(getpid());
}

uint64_t get_address_of_port_OWO(uint64_t proc, mach_port_t port)
{
    uint64_t const task_addr = ReadKernel64(proc + koffsetS3(KSTRUCT_OFFSET_PROC_TASK));
    uint64_t const itk_space = ReadKernel64(task_addr + koffsetS3(KSTRUCT_OFFSET_TASK_ITK_SPACE));
    uint64_t const is_table = ReadKernel64(itk_space + koffsetS3(KSTRUCT_OFFSET_IPC_SPACE_IS_TABLE));
    uint64_t const port_addr = ReadKernel64(is_table + (MACH_PORT_INDEX(port) * koffsetS3(KSTRUCT_SIZE_IPC_ENTRY)));
    return port_addr;
}
...
kptr_t IOMalloc(vm_size_t size) {
    ...
    if (ret != KPTR_NULL) ret = zm_fix_addr1(ret);
out:;
    return ret;
}
...
uint64_t get_iodtnvram_obj(void) {
    ...
    if (IODTNVRAMObj == 0) {
        ...
        IODTNVRAMObj = ReadKernel64(nvram_up + koffsetS3(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT));
