...
int
init_kernel1(addr_t base, const char *filename)
{
    size_t rv;
    uint8_t buf[0x4000];
    unsigned i, j;
...
#ifdef __ENVIRONMENT_IPHONE_OS_VERSION_MIN_REQUIRED__
#define close(f)
    rv = kread(basefromkernelmemory, buf, sizeof(buf));
    if (rv != sizeof(buf)) {
        return -1;
    }
    ...
    kernel_mh = kernel + basefromkernelmemory - min;
...
addr_t
find_register_value1(addr_t where, int reg)
{
    addr_t val;
    addr_t bof = 0;
    where -= kerndumpbase;
...
addr_t
find_reference1(addr_t to, int n, int prelink)
{
    addr_t ref, end;
    ...
    to -= kerndumpbase;
    do {
        ref = xref64(kernel, basefromkernelmemory, end, to);
        if (!ref) {
            return 0;
        }
        base = ref + 4;
...
addr_t
find_strref1(const char *string, int n, int prelink)
{
    uint8_t *str;
    ...
    str = boyermoore_horspool_memmem(kernel + basefromkernelmemory, size, (uint8_t *)string, strlen(string));
    if (!str) {
        return 0;
    }
    return find_reference1(str - kernel + kerndumpbase, n, prelink);
}

/****** fun *******/

addr_t find_add_x0_x0_0x40_ret1(void) {
	addr_t off;
	uint32_t *k;
...
uint64_t find_allproc1(void) {
	...
	addr_t ref = find_strref1("\"pgrp_add : pgrp is dead adding process\"", 1, 0);
	if (!ref) {
		return 0;
	}
	ref -= kerndumpbase;
...
uint64_t find_copyout1(void) {
	// Find the first reference to the string
	addr_t ref = find_strref1("\"%s(%p, %p, %lu) - transfer too large\"", 2, 0);
	if (!ref) {
		return 0;
	}
	ref -= kerndumpbase;
...
uint64_t find_rootvnode1(void) {
	// Find the first reference to the string
	addr_t ref = find_strref1("/var/run/.vfs_rsrc_streams_%p%x", 1, 0);
	if (!ref) {
		return 0;
	}
	ref -= kerndumpbase;
...
addr_t find_trustcache1(void) {
    addr_t call1, call2, call3, call4, call5;
    addr_t func1, func2, func4;
    addr_t val;
    addr_t amfiUC_inTrustCache = find_strref1("%s: only allowed process can check the trust cache", 1, 1); // Trying to find AppleMobileFileIntegrityUserClient::isCdhashInTrustCache
...
addr_t find_amficache1(void) {
	addr_t call, func, bof, val;
	addr_t ref = find_strref1("com.apple.MobileFileIntegrity", 1, 1);
	if (!ref) {
		return 0;
	}
	ref -= kerndumpbase;
...
addr_t find_realhost1(void) {
    uint64_t val = kerndumpbase;

    addr_t ref1 = find_strref1("\"ipc_init: kmem_suballoc of ipc_kernel_copy_map failed\"", 1, 0);
    ref1 -= kerndumpbase;
    addr_t ref2 = find_strref1("\"ipc_host_init\"", 1, 0);
    ref2 -= kerndumpbase;
...
addr_t find_zone_map_ref1(void) {
    // \"Nothing being freed to the zone_map. start = end = %p\\n\"
    uint64_t val = kerndumpbase;

    addr_t ref = find_strref1("\"Nothing being freed to the zone_map. start = end = %p\\n\"", 1, 0);
    ref -= kerndumpbase;
    ...
    ref = step64_back(basefromkernelmemory, ref, 30, INSN_ADRP);
...
uint64_t find_zone_map1(void) {
    uint64_t zone_map_kptr = find_zone_map_ref1();
