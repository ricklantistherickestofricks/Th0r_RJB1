...
uint64_t cached_task_self_addr1 = 0;
bool found_offs = false;
uint64_t task_self_addr1()
{
    if (cached_task_self_addr1 == 0) {
        cached_task_self_addr1 = find_port_address(mach_task_self(), MACH_MSG_TYPE_COPY_SEND);//have_kmem_read() && found_offs ? get_address_of_port(getpid(), mach_task_self()) : find_port_address(mach_task_self(), MACH_MSG_TYPE_COPY_SEND);
        LOG("task self: 0x%llx", cached_task_self_addr1);
    }
    return cached_task_self_addr1;
}
